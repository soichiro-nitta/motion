...
- cleanup が必要な場合は、`useEffectAsync(async (c) => { ... })` の `c(() => ...)` で登録してください。
...
- `useEffectAsync(effect, deps)`
  ...
  - cleanup は `effect` の戻り値ではなく、第一引数 `c` で登録します。
  - `c` は同期的に登録されるため、`await` 中に unmount されても登録済み cleanup は確実に実行されます。

```ts
useEffectAsync(async (c) => {
  const id = window.setInterval(() => {
    // do something
  }, 1000)
  c(() => window.clearInterval(id))
